        ...
        connection.query(`INSERT INTO tbl_user SET ?`, params, (err, result) => {  
            if (err) {
                ...
                if (req.referralPointsId == 0) {
                    const jwtToken = jwt.sign({ id: result.insertId }, process.env.JWT_SECRET);
                    result.jwtToken = jwtToken
                    resolve(result);
                }else{
                    connection.query('UPDATE tbl_user SET points = points + ' + req.referralPoints + ' WHERE id ="' + req.referralPointsId + '"', function(err1, result1) {
                        const jwtToken = jwt.sign({ id: result.insertId }, process.env.JWT_SECRET);
                        result.jwtToken = jwtToken
                        resolve(result);
                    })
